...
yarrr::pirateplot(formula = word_count ~ Released,
           data = word_summary,
           xlab = NA,
...
relative_polarity_over_time <- songs_polarity_year %>%
  ...
  theme_songs() + theme(plot.title = element_text(size = 11)) +
  labs(x = NULL, y = NULL) +
  ggtitle("Percent Postiive Over Time")

gridExtra::grid.arrange(polarity_over_time, relative_polarity_over_time, ncol = 2)
...
#another chart with same thing
#!!!needs to be finished
decade_sentiment_nrc %>%
  inner_join(total_sentiment_decade, by = "decade") %>%
  mutate(percent = sentiment_count / decade_total * 100) %>%
  filter(!decade %in% "1950s") %>%
  select(-sentiment_count, -decade_total) %>%
  ggplot(aes(x = as.factor(decade), y = percent, group = sentiment, color = sentiment)) +
  geom_line()

#another idea - find the top n songs for each sentiment and negative/positive
###to do
...
#stopped at pairwise correlations